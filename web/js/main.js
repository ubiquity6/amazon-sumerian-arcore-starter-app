(function(
    CanvasWrapper,
    WebGLSupport,
    ARKitBridge,
    ARCoreBridge
) {
    'use strict';

    var sumerianRunner = null;

    // Read from the page's query string. When true, hides all UI elements, makes the background transparent,
    // and enables the alpha channel on the canvas used by the Sumerian engine.
    var arMode = false;

    // Set to either ARKitBridge or ARCoreBridge based on the platform.
    var arBridge;

    function setup(scene, loader) {
        if (arMode) {
            arBridge.setArSystem(sumerianRunner.world.getSystem('ArSystem'), sumerianRunner.world);
        }

        // Application code goes here!

        /*
         To get a hold of entities, one can use the World's selection functions:
         var allEntities = sumerianRunner.world.getEntities();                  // all
         var entity      = sumerianRunner.world.by.name('EntityName').first();  // by name
         */
    }

    /**
     * Entry point. Gets called after the script is loaded and displays the
     * fallback if no WebGL is found, adds event listeners and starts loading
     * the scene into the engine.
     *
     * @return {Promise}
     */
    function init() {
        if (!checkForWebGLSupport()) {
            return;
        }

        arMode = getQueryStringValue('arMode') === 'true';

        if (arMode) {
            document.body.style.backgroundColor = 'transparent';
            document.getElementById('bottom-bar').style.display = 'none';
            arBridge = WebGLSupport.getBrowser() === WebGLSupport.BROWSERS.IOS ? ARKitBridge : ARCoreBridge;
        }

        // Init the SumerianEngine
        initSumerian();
        var world = sumerianRunner.world;
        var renderer = sumerianRunner.renderer;

        preventBrowserInconsistencies();
        addButtonListeners();

        // Crazy hack to make orientation change work on the webview in iOS.
        sumerian.SystemBus.addListener('sumerian.viewportResize', function() {
            var dpx = sumerianRunner.renderer.devicePixelRatio;
            renderer.domElement.style.width = '1px';
            renderer.domElement.style.height = '1px';
            renderer.domElement.offsetHeight;
            renderer.domElement.style.width = '';
            renderer.domElement.style.height = '';
        });

        // Load the scene
        loadScene().then(function(loaderAndScene) {
            var loader = loaderAndScene.loader;
            var scene = loaderAndScene.scene;

            world.process();

            if (sumerian.Renderer.mainCamera) {
                renderer.checkResize(sumerian.Renderer.mainCamera);
            }

            return setup(scene, loader);
        }).then(function() {
            (new sumerian.EntityCombiner(world)).combine();
            world.process();
            return prepareMaterials();
        }).then(function() {
            show('canvas-screen');
            hide('loading-screen');
            CanvasWrapper.show();
            CanvasWrapper.resize();
            setupWebVR();
            sumerianRunner.startGameLoop();

            renderer.domElement.focus();

        }).then(null, function(error) {
            // If something goes wrong, 'error' is the error message from the engine.
            console.error(error);
        });
    }

    /**
     * Setup WebVR system
     */
    function setupWebVR() {
        let vrSystem = sumerianRunner.world.getSystem('VRCameraRigSystem');

        // TODO: Binding issues finding vrSystem in the plain JavaScript addEventListener.
        // Yuan will refactor this to not use jQuery when we migrate this class to SumerianNeoEngine.
        const vrIcons = document.querySelector('#vr-icons');

        if (!sumerian.VRCameraRigSystem.canPresent) {
            return;
        }

        const inVR = document.querySelector('#in-vr');
        const enterVR = vrIcons.querySelector('#enter-vr');
        const exitVR = vrIcons.querySelector('#exit-vr');

        enterVR.addEventListener('click', () => {
            try {
                vrSystem.startVR();
            } catch (e) {
                alert('Unable to start WebVR System: ' + e.message);
            }
        });

        exitVR.addEventListener('click', () => {
            try {
                vrSystem.stopVR();
            } catch (e) {
                alert('Unable to stop WebVR System: ' + e.message);
            }
        });

        const updateUI = () => {
            if (sumerian.VRCameraRigSystem.isPresenting) {
                // TODO: allow the scene to specify what should be shown in the
                // browser when in vr. For now we show the sterioscopic rendering
                // if the display supports it, otherwise we hide the canvas because
                // it will freeze. https://issues.labcollab.net/browse/SUMERIAN-2663
                if (!vrSystem.hasExternalDisplay()) {
                    inVR.classList.remove('hide');
                    inVR.classList.add('show');
                }

                exitVR.classList.remove('hide-vr-icon');
                exitVR.classList.add('show-vr-icon');

                enterVR.classList.remove('show-vr-icon');
                enterVR.classList.add('hide-vr-icon');
            } else {
                inVR.classList.remove('show');
                inVR.classList.add('hide');

                exitVR.classList.remove('show-vr-icon');
                exitVR.classList.add('hide-vr-icon');

                enterVR.classList.remove('hide-vr-icon');
                enterVR.classList.add('show-vr-icon');
            }
        };

        sumerian.SystemBus.addListener('sumerian.VRPresentationStarted', updateUI);
        sumerian.SystemBus.addListener('sumerian.VRPresentationStopped', updateUI);
        updateUI();
    }

    /**
     * Preloads the shaders used by the materials in the scene and then preloads
     * those materials.
     *
     * @return {Promise}
     */
    function prepareMaterials() {
        var renderer = sumerianRunner.renderer;
        var renderSystem = sumerianRunner.world.getSystem('RenderSystem');
        var entities = renderSystem._activeEntities;
        var lights = renderSystem.lights;

        return renderer.precompileShaders(entities, lights).then(function() {
            return renderer.preloadMaterials(entities);
        })
    }

    /**
     * Initializes the Sumerian Engine and all the systems.
     */
    function initSumerian() {
        // Create typical Sumerian application.
        var params = {
            "alpha": false,
            "useDevicePixelRatio": false,
            "manuallyStartGameLoop": true,
            "antialias": true,
            "logo": true
        };
        params.logo = false; // Handled in the html.
        params.alpha = arMode || params.alpha;
        sumerianRunner = new sumerian.SumerianRunner(params);

        var stateMachineSystem = new sumerian.StateMachineSystem(sumerianRunner);
        sumerianRunner.world
            .add(new sumerian.AnimationSystem())
            .add(stateMachineSystem)
            .add(new sumerian.HtmlSystem(sumerianRunner.renderer))
            .add(new sumerian.Dom3dSystem(sumerianRunner.renderer))
            .add(new sumerian.TimelineSystem())
            .add(new sumerian.PhysicsSystem())
            .add(new sumerian.ColliderSystem())            
            .add(new sumerian.ParticleSystemSystem());

        if (arMode) {
            sumerianRunner.world.add(new sumerian.ArSystem(arBridge));
        }

        sumerianRunner.addStats();

        stateMachineSystem.play();
    }

    /**
     * Loads the scene.
     *
     * @return {Promise}
     *         A promise which is resolved when the scene has finished loading.
     */
    function loadScene() {
        // The dynamic loader takes care of loading the data.
        var loader = new sumerian.DynamicLoader({
            world: sumerianRunner.world,
            rootPath: 'res'
        });
        sumerianRunner.world.loader = loader;

        return loader.load('root.bundle').then(function(bundle) {
            var scene = getFirstSceneFromBundle(bundle);
            var alphaEnabled = false;

            // Disable all the skyboxes if the background is transparent.
            if (alphaEnabled) {
                Object.keys(bundle)
                    .filter(function(k) {
                        return /\.skybox$/.test(k);
                    })
                    .forEach(function(k) {
                        var v = bundle[k];
                        v.box.enabled = false;
                    });
            }

            if (!scene || !scene.id) {
                console.error('Error: No scene in bundle'); // Should never happen.
                return null;
            }

            // Setup the canvas configuration (sizing mode, resolution, aspect
            // ratio, etc).
            var canvasConfig = scene ? scene.canvas : {};
            CanvasWrapper.setup(sumerianRunner.renderer.domElement, canvasConfig);
            CanvasWrapper.add();
            CanvasWrapper.hide();

            return loader.load(scene.id, {
                    preloadBinaries: true,
                    progressCallback: onLoadProgress
                })
                .then(function(scene) {
                    return {
                        scene: scene,
                        loader: loader
                    };
                });
        });
    }

    /**
     * Gets the first scene from the specified bundle.
     *
     * @param {object} bundle
     *        Bundle containing all the entities and assets in the scene.
     *
     * @return {object}
     *         The configuration object of the first scene that was found in
     *         the bundle.
     */
    function getFirstSceneFromBundle(bundle) {
        function isSceneId(id) {
            return /\.scene$/.test(id);
        }

        for (var id in bundle) {
            if (isSceneId(id)) {
                return bundle[id];
            }
        }

        return null;
    }

    /**
     * Callback for the loading screen.
     *
     * @param  {number} handled
     * @param  {number} total
     */
    function onLoadProgress(handled, total) {
        var loadedPercent = (100 * handled / total).toFixed();
        document.getElementById('progress').style.width = loadedPercent + '%';

        window.postMessage({
            handled: handled,
            total: total,
            loadedPercent: loadedPercent
        }, '*')
    }

    /**
     * Adds an event listener to the buttons on the bottom bar.
     */
    function addButtonListeners() {
        var maximizeButton = document.getElementById('maximize-button');
        maximizeButton.addEventListener('click', maximize);
        maximizeButton.addEventListener('touchstart', maximize);

        var muteButton = document.getElementById('mute-button');
        muteButton.addEventListener('click', toggleMute);
        muteButton.addEventListener('touchstart', toggleMute);
    }

    /**
     * Requests the browser to go into fullscreen mode.
     */
    function maximize() {
        var element = document.getElementById('canvas-outer');
        if (element.requestFullscreen) {
            element.requestFullscreen();
        } else if (element.msRequestFullscreen) {
            element.msRequestFullscreen();
        } else if (element.mozRequestFullScreen) {
            element.mozRequestFullScreen();
        } else if (element.webkitRequestFullscreen) {
            element.webkitRequestFullscreen();
        }
    }

    /**
     * Mute sounds. Returns true if the new mute state is muted, and false otherwise.
     */
    function toggleMute() {
        var soundSystem = sumerianRunner.world.getSystem('SoundSystem');
        var muteButton = document.getElementById('mute-button');
        if (soundSystem.muted) {
            soundSystem.unmute();
            muteButton.classList.add('icon-sound');
            muteButton.classList.remove('icon-sound-mute');
        } else {
            soundSystem.mute();
            muteButton.classList.add('icon-sound-mute');
            muteButton.classList.remove('icon-sound');
        }
    }

    /**
     * Prevent browser peculiarities from messing with our controls.
     */
    function preventBrowserInconsistencies() {
        document.body.addEventListener('touchstart', function(event) {
            function isLink(el) {
                return el.nodeName === 'A';
            }

            if (isLink(event.target)) {
                return;
            }

            var node = event.target.parentElement;
            for (var i = 0; i < 5; i++) {
                if (!node) {
                    break;
                }
                if (isLink(node)) {
                    return;
                }
                node = node.parentElement;
            }

            event.preventDefault();
        }, false);
    }

    /**
     * Checks if WebGL is supported by the current browser and, if not, shows
     * the fallback.
     */
    function checkForWebGLSupport() {
        var errorObject = WebGLSupport.check();

        if (errorObject.error === WebGLSupport.ERRORS.NO_ERROR) {
            show('loading-screen');
            return true;
        } else {
            show('fallback');
            hide('loading-screen');
            return false;
        }
    }

    function getQueryStringValue(key) {
        var queryString = window.location.search.slice(1);
        var parameters = queryString.split('&');

        for (let i = 0; i < parameters.length; ++i) {
            let keyValuePair = parameters[i].split('=');
            if (keyValuePair[0] == key) {
                return keyValuePair[1];
            }
        }

        return null;
    }

    /**
     * Converts camelCase (js) to dash-case (html)
     */
    function camel2dash(str) {
        return str.replace(/([a-z])([A-Z])/g, '$1-$2').toLowerCase();
    }

    function show(id) {
        var classList = document.getElementById(id).classList;
        classList.add('visible');
        classList.remove('hidden');
    }

    function hide(id) {
        var classList = document.getElementById(id).classList;
        classList.remove('visible');
        window.setTimeout(function() {
            classList.add('hidden');
        }, 500);
    }

    //--------------------------------------------------------------------------

    init();
})(CanvasWrapper, WebGLSupport, ARKitBridge, ARCoreBridge);